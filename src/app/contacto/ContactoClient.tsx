        ...
        <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div className="bg-white p-8 rounded-lg shadow-lg flex flex-col items-center text-center">
                        ...
                        <div className="space-y-6 w-full max-w-sm">
                            <div className="flex items-center justify-center gap-4 text-gray-800"><FaMapMarkerAlt className="text-blue-600 text-3xl" /><div><p className="font-semibold text-lg">Dirección:</p><p className="text-md">{address}</p></div></div>
                            <div className="flex items-center justify-center gap-4 text-gray-800"><FaPhone className="text-blue-600 text-3xl" /><div><p className="font-semibold text-lg">Teléfono:</p><a href={`tel:${phoneNumber}`} className="text-md hover:underline">{phoneNumber}</a></div></div>
                            <div className="flex items-center justify-center gap-4 text-gray-800"><FaEnvelope className="text-blue-600 text-3xl" /><div><p className="font-semibold text-lg">Correo Electrónico:</p><a href={`mailto:${emailAddress}`} className="text-md hover:underline">{emailAddress}</a></div></div>
                        </div>
                        <div className="mt-8">
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-lg flex flex-col items-center">
                        ...
                        <div className="w-full h-80 bg-gray-200 rounded-lg overflow-hidden flex items-center justify-center text-gray-500 text-sm">
                            <iframe
                                ...
                                loading="lazy" // Carga diferida del mapa
                                referrerPolicy="no-referrer-when-downgrade" title="Ubicación de Tersus Clean"
                            ></iframe>
                        </div>
                        <a href={googleMapsLink} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex items-center bg-blue-600 text-white px-6 py-3 rounded-full font-bold text-lg hover:bg-blue-700 transition-colors shadow-lg"><FaMapMarkerAlt className="mr-2" /> Abrir en Google Maps</a>
                    </div>
                </div>
            </div>
            <div className="bg-blue-800 py-16 md:py-24 text-white text-center">
                <div className="container mx-auto px-4">
                    ...
                    <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="bg-green-500 text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-green-600 transition-colors shadow-lg flex items-center justify-center max-w-xs mx-auto"><FaWhatsapp className="mr-2" /> Escríbenos por WhatsApp</a>
                </div>
            </div>
        </>
    );
